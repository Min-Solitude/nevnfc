...
const Home = () => {
  ...
  if(isMobile) return(
    <motion.div className=' h-screen overflow-hidden'>
      <motion.div className='rounded-xl bg-white h-full flex gap-8  flex-col items-center justify-center'
        initial={{ opacity: 0, scale: 0, rotate: 360 }}
        animate={{ opacity: 1, scale: 1, rotate: 0 }}
        transition={{ duration: 1.5,  type: 'spring', stiffness: 100 }}
      >
        <div className='w-[15rem] h-[15rem] rounded-lg banner-gradient p-2'>
          <img src='https://i.pinimg.com/564x/07/cf/fa/07cffa5e337dbe0b7f350af68fcb2335.jpg' alt='nev' className='w-full h-full rounded-lg'/>
        </div>
        <motion.button className='banner-gradient p-1 rounded-full'
          whileTap={{ scale: .9 }}
  ...
  return (
    <div className=' bg-white overflow-hidden flex h-screen relative justify-center items-center'>
      {
        isChangeLayout ? dataCard.map((item, index) => 
          item.cardPosition ? (
            <motion.div key={index} className='w-[15rem] p-2 h-[20rem] absolute rounded-lg cursor-pointer bg-[#fafafa] shadow-md border border-gray-200'
        ...
        >
          ...
          <div className='py-2'>
            ...
            <motion.div key={index} className='w-[15rem] p-2 h-[20rem] absolute rounded-lg shadow-md cursor-pointer bg-[#fafafa]'
          ...
          <motion.div className='fixed items-center left-0 w-full bg-white shadow-md flex justify-between p-4 top-0'
            initial={{ opacity: 0, scale: 0 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 1.5,  type: 'spring', stiffness: 100 }}
          >
            <motion.button className={`flex justify-center relative items-center p-1 rounded-lg ${isDropMenu && 'banner-gradient text-white'}`}
              whileTap={{ scale: .9 }}
                  ...
                  <motion.div className='absolute p-2 flex flex-col gap-2 rounded-xl left-0 -bottom-8 translate-y-[100%] bg-white shadow-sm border border-gray-100 w-[20rem]'>
                    {
                      dataCard.map((item, index) => (
                        <Link className=' rounded-lg p-1 hover:bg-gray-100 flex flex-col items-start text-black' to={item.link} key={index}>
            ...
            <div><Logo/></div>
            <motion.button className={`flex justify-center relative items-center p-1 rounded-lg ${isShowQrCode && 'banner-gradient text-white'}`}
              whileTap={{ scale: .9 }}
            ...
            {
              isShowQrCode && (
                <motion.div className='absolute p-1 rounded-xl right-0 -bottom-8 translate-y-[100%] banner-gradient'
      ...
      <motion.div className='w-[50%] flex items-center justify-center p-2  rounded-xl relative banner-gradient shadow-md'
        initial={{ opacity: 0, scale: 0 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1.5,  type: 'spring', stiffness: 100 }}
      >
        <div className='bg-white rounded-xl p-4'>
          <img src='https://uploads-ssl.webflow.com/61a2b0883305d337048a4219/61a30c1d6bff265c8a5cc71e_Creator%20Circles.png' alt='nev' 
          className=''
          />
          <div className='flex mt-4 xl:mt-8 flex-col items-center'>
            ...
            <p className='text-gray-500 text-center text-[0.9rem] font-light'>Experiencing all our services will be a blessing for you</p>
            <motion.button className='text-[0.9rem] mt-4 mb-2 banner-gradient rounded-full p-1 '
              whileTap={{ scale: .9 }}
